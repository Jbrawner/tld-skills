...
> **Jira path:** the skip target is any `To Do`-category status (prefer one named `Skipped` if the workflow has it, else `Backlog`/`To Do`). Move the sub-task with `getTransitionsForJiraIssue` → `transitionJiraIssue` (status changes are transitions, not field writes). Nothing else changes: there is no `## Order` text, and the sub-task keeps its rank position so it can be resumed. See docs/JIRA.md § Statuses. The Linear steps below do not apply.
