...
> **Jira path:** status changes go through a workflow transition, not a field write — call `getTransitionsForJiraIssue` for the sub-task and `transitionJiraIssue` to the Done-category status (see docs/JIRA.md § Statuses). For step 5, "what's next" is the next-ranked unfinished **Sub-task** under the same milestone Story (`parent = "<storyKey>" ORDER BY Rank ASC`, skip Done/Canceled and sub-tasks In Progress for someone else), not an `## Order` walk; read its labels via `getJiraIssue` and apply label overrides (step 7) via `editJiraIssue`. If no unfinished sub-task remains, run the milestone gate as below.
