...
| **Release QA contract** | `docs/release-qa/README.md`, `docs/release-qa/FLOW_MATRIX.md` and `docs/release-qa/runs/TEMPLATE.md` in the repo | Only `/release-qa` needs them. They are the whole product-specific half of the walk: the README holds the project config, the matrix holds the flows, the template holds the run-record shape. Without all three the skill stops and names the missing file rather than inventing a walk. Every other skill is unaffected. |
| **Week plan contract** | `docs/plans/README.md` and `docs/plans/TEMPLATE.md` in the repo, plus one `<date>-week.md` per week anywhere under `docs/plans/` (tracked or not, the contract says which) | Only `/week-plan` needs them. The README holds the project config and the shape of a week, the template holds the shape of a week file, and the dated file holds that week's order and reasons. Status is never in the file: `week-plan/plan-status.sh` reads it live from Jira by status category, so the skill is Jira-only until an adapter lands. Without the README the skill stops and names it. Every other skill is unaffected. |
