...
- Read `.tld/campaign.md` at the repo root first for project context: the issue tracker, the Jira **project key** (the `Ticket prefix` field — e.g. `WEB`, `API`, `APP`), the display `Project name`, the stack directories, the test commands, and the commit pattern. This is the source of truth for which Jira project the ticket, branch, and board point at. If `.tld/campaign.md` is absent, fall back to `docs/JIRA.md`.
...
- Read `.tld/campaign.md` at the repo root before attempting Jira work. From its **Project** section take the issue tracker, the **Jira project key** (the `Ticket prefix` field — e.g. `WEB`, `API`, `APP`), and the display `Project name`; from **Stack** the directories; from **Test Commands** the commands; from **Commit format** the pattern.
- The Jira project key (ticket prefix) is what the ticket is created in and what the board and branch point at — e.g. prefix `WEB` → board `https://your-workspace.atlassian.net/jira/software/c/projects/WEB/boards/12`. Note `Project name` may be a friendly label (e.g. `Acme`) and is not always the key — always use the ticket prefix as the key.
