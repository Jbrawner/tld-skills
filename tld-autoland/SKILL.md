...
6. **Trigger CI (merge-eligible tickets only).** If the repo gates CI on a label and/or draft state (e.g. lab-inventory's `ci:run` on a non-draft PR), mark the PR ready and add the label now — this is the pre-merge moment: the branch is full-auto verified and §1.7 merges the instant checks pass, so CI runs exactly once. Skip for a park-with-PR ticket — leave it draft and unlabeled so it does not burn CI while it waits for a human.
